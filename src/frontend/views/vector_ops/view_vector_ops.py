                        ...
                        with ui.tab_panel('add_sub').classes('p-0'):
                            with ui.row().classes('w-full justify-between items-center mb-4 panel-card p-4'):
                                with ui.row().classes('gap-4 items-center'):
                                    ...
                                    self.op_select = ui.select({'add': 'Suma (v1 + v2)', 'subtract': 'Resta (v1 - v2)'}, value=self.add_sub_operation, on_change=lambda e: setattr(self, 'add_sub_operation', e.value)).classes('w-48')
                                self.strict_check = ui.checkbox('Estricto (No auto-transponer)', value=self.add_sub_strict, on_change=lambda e: setattr(self, 'add_sub_strict', e.value))
