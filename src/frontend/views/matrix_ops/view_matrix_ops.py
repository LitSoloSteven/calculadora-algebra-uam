...
class MatrixOpsUI:
    ...
    def reset_resultados(self):
        ...
        with self.contenedor_resultados:
            ui.icon('calculate', size='3rem').style('color: var(--text-placeholder); opacity: 0.5;')
            ui.label('Los resultados aparecerán aquí').classes('mt-4 text-sm font-medium').style('color: var(--text-placeholder);')
            ...
            if respuesta.get("status") == "ERROR":
                    ...
                    for i, step in enumerate(respuesta["segment_steps"]):
                        with ui.column().classes('w-full panel-card p-6 mb-4'):
                            ...
                            with ui.row().classes('w-full items-center justify-center gap-4 py-4'):
                                ui.html(f'<div class="math-label text-xl overflow-x-auto">$$ {step.get("symbolic_matrix_latex", "")} $$</div>')
                                ui.icon('arrow_forward', size='md').classes('text-sec')
                                ui.html(f'<div class="math-label text-xl overflow-x-auto">$$ {step.get("result_matrix_latex", "")} $$</div>')
                            
                            if step.get("cell_by_cell_steps"):
                                with ui.expansion('Ver detalle celda a celda', icon='visibility').classes('w-full mt-2').props('header-class="font-medium text-sec"'):
                                    for cell_step in step["cell_by_cell_steps"]:
                                        ui.html(f'<div class="math-label w-full py-1">$$ {cell_step.get("detail_latex", "")} $$</div>')
                ...
                if respuesta.get("final_variable") and respuesta.get("result_matrix_latex"):
                    ...
                    with ui.row().classes('w-full justify-center items-center panel-card p-6 overflow-x-auto'):
                        ui.html(f'<div class="math-label text-2xl font-bold">$$ {respuesta["final_variable"]} = {respuesta["result_matrix_latex"]} $$</div>')

        btn.props('loading=false')
        ui.run_javascript('setTimeout(() => { if (window.MathJax) { MathJax.typesetClear(); MathJax.typesetPromise(); } }, 100);')
        ui.run_javascript('setTimeout(() => { const res = document.getElementById("' + str(self.contenedor_resultados.id) + '"); if(res) res.classList.remove("animate-slide-up"); }, 600);')
        ui.run_javascript("setTimeout(() => { document.querySelector('.min-h-\\\\[400px\\\\]').scrollIntoView({behavior: 'smooth', block: 'start'}) }, 300);")
    ...
    def build(self):
        ...
        with ui.column().classes('w-full max-w-7xl mx-auto p-6 mt-4'):
            ...
            with ui.row().classes('w-full no-wrap items-stretch gap-8 mb-8'):
                # Panel izquierdo (Matrices y expresión)
                with ui.column().classes('flex-1 w-1/2 min-w-[350px]'):
                ...
                with ui.column().classes('flex-1 w-1/2 min-w-[350px]'):
                    self.contenedor_resultados = ui.column().classes('w-full panel-card p-6 items-center justify-center min-h-[400px]')
